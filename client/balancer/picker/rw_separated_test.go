...
func TestRwSeperatedRoundBalanced_Pick(t *testing.T) {
	...
	t.Run("leader", func(t *testing.T) {
		ast := assert.New(t)
		for i := 0; i < times; i++ {
			rs, err := picker.Pick(balancer.PickInfo{
	...
	t.Run("follower", func(t *testing.T) {
		...
		expected := 0
		for i := 0; i < times; i++ {
			actual, err := picker.Pick(balancer.PickInfo{
				FullMethodName: readTest,
				Ctx:            nil,
			})
			ast.NoError(err)
			ast.Equal(addrToSC[followerAddrs[expected%len(followerAddrs)]], actual.SubConn)
			expected++
		}
	})
...
func TestRwSeperatedRoundBalanced_SingleNodeCluster(t *testing.T) {
	...
	ast.NotPanics(func() {
		...
		ast.NoError(err)
		ast.Equal(actual.SubConn, sc)

		actual.Done(balancer.DoneInfo{})
		return
	})
